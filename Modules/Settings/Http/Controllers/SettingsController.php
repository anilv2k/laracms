...
public function store(Request $request)
    {
        ...
        foreach ($request->input('key') as $key => $value) {
            ...
            $sett = Setting::updateOrCreate(
                ['name' => $key, 'type' => $request->input('type')],
                ['value' => $value]
            );
        }
    ...
    */
    public static function get_admin_navigation(){
	   ...
	   $navs = json_decode(AdminNavigation::where('status',1)->whereJsonContains('roles', ['role' => $user->role])->orderBy('nav_order')->get(),1);


   // $navs = json_decode(AdminNavigation::where('status',1)->whereRaw("find_in_set($user->role,roles)")->orderBy('nav_order')->get(),1);
	...
	public function themes(){
        ...
        $data['themes'] = $template;
         ...
         $sett = new Setting();
         $data['active_theme']= $sett->where('name','site_theme')->first()->value;
