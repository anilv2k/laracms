...
class UserController extends Controller
{
    ...
    {   
		...
		/* $rules = array(
		'email-address'=>'required|unique:content',
        ...
        if(Auth::guard('web')->attempt($cred,true)){
	      ...
	      // echo  $college->university->weburl;
			
		  // redirect($college->university->weburl.'/user/dashboard');
            ...
            $response=array('status'=>true,'weburl'=>$college->university->weburl);
            echo json_encode($response);
        }
        else{
			
			$response=array('message'=>'Invalid email/password.','status'=>$college->university->weburl);
            echo json_encode($response);
        }
    ...
    {   
	    /*$states = \Modules\User\Entities\State::find(1); //->users()->where('id', 1);
		...
		foreach($formdata['colleges'] as $v){
	    ...
	    Datatable::setFields(array('id','name','email','created_at'))->setModule('user')->setSubject('user')->showFilters('users');;
		Datatable::displayFields(array('id'=>'User ID','name'=>'User Name'))->setListurl('user/ajax_list')->setOptionurl('user/ajax_options')->showActions();
			
     	return Theme::uses('admin')->view('user::list',$data);
	}
    ...
    {    
		...
		if($option=='addeditSave'){
            ...
            echo json_encode($json_data);
			   ...
			   Datatable::setModule('user')->viewFiles($option,$formdata);
		}
    ...
    {   
	   // return bcrypt('anil123'); 
	    ...
	    return Theme::view('user::user_registration',$data);
    }
